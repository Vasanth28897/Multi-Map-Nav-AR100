...
WormholeManager::WormholeManager(const string &db_path)
{
...
pair<double, double> WormholeManager::getWormholeToMap(const string &current_map, const string &target_map)
{
  double x =  -9999 , y = -9999;

  string sql = "SELECT from_x, from_y FROM wormholes_coord WHERE from_map = ? AND to_map = ?;";
  sqlite3_stmt *stmt;
