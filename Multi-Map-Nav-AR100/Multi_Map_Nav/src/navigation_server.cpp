...
NavigationServer::NavigationServer(const string &db_path)
...
bool NavigationServer::move_base_to(double x, double y, double yaw)
{
  ...
  if (!ac.waitForServer(Duration(5.0)))
  {
  ...
  mb_goal.target_pose.header.stamp = Time::now();
...
void NavigationServer::execute(const multi_map_nav::NavToGoalGoalConstPtr &goal)
{
  ROS_INFO("Received goal: target_map: %s, target_x: %f, target_y: %f",
            goal->target_map.c_str(), goal->target_x, goal->target_y);

  double yaw = M_PI / 2;
  multi_map_nav::NavToGoalResult result;
  ...
  if (current_map_ != "map2")
  {
    auto to_intermediate = wormhole_manager_.getWormholeToMap(current_map_, "map2");
    if (to_intermediate.first == -9999 || to_intermediate.second == -9999)
    {
      ROS_ERROR("No wormhole from %s to map2", current_map_.c_str());
      result.success = false;
      result.message = "No wormhole to map2";
      as_.setAborted(result);
      return;
    }

    ROS_INFO("Moving from %s to map2 via (%f, %f)", current_map_.c_str(), to_intermediate.first, to_intermediate.second);
    ...
    map_switcher_.switchToMap("map2");
    current_map_ = "map2";
  }

  auto to_final = wormhole_manager_.getWormholeToMap("map2", goal->target_map);
  if (to_final.first == -9999 || to_final.second == -9999)
  {
    ROS_ERROR("No wormhole from map2 to %s", goal->target_map.c_str());
    result.success = false;
    result.message = "No wormhole from map2 to target";
    as_.setAborted(result);
    return;
  }

  ROS_INFO("Moving from map2 to %s via (%f, %f)", goal->target_map.c_str(), to_final.first, to_final.second);
...
{
    init(argc, argv, "navigation_server_node");
    NodeHandle nh;

    string db_path;
    nh.param<string>("wormhole_db_path", db_path,
                          package::getPath("multi_map_nav") + "/database/wormholes.db");
